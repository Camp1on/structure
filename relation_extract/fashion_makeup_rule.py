...
from structure.relation_extract.constant import ALL_RELATION

# 使用近邻的思想进行关系抽取
# 1、同一行保留最先出现的品类，其它忽略（todo 可尝试同一行的近邻方法）
# 2、以品类实体对所有句子进行切分，第i个品类词到第i+1个品类词之前的属性都是描述第i个品类ci
class FashionMakeupRuleExtractor:
    ...
    def predict(self, text, entity_rst):
        ...
        for group in sent_group:
            ...
            for s in cur_subjects:
                rst.append({'s': s, 'p': ALL_RELATION['描述'], 'o': cur_object, 'score': 1.0})
        return rst
    ...
    def predict_strict(self, text, entity_rst):
        ...
        for i in range(len(entity_rst)-1):
            ...
            flag2 = entity_rst[i]['type'] != self.PINLEI and entity_rst[i+1]['type'] == self.PINLEI
            if flag1 and flag2:
                s = entity_rst[i]
                o = entity_rst[i+1]
                rst.append({'s': s, 'p': ALL_RELATION['描述'], 'o': o, 'score': 1.0})
                j = i - 1
                after = entity_rst[i]
                while j >= 0:
                    if entity_rst[j]['type'] == self.PINLEI:
                        break
                    if entity_rst[j]['endPosition'] == after['startPosition']:
                        rst.append({'s': entity_rst[j], 'p': ALL_RELATION['描述'], 'o': o, 'score': 1.0})
                        after = entity_rst[j]
                    j -= 1

            # 后序谓语模式
            flag3 = entity_rst[i]['type'] == self.PINLEI and entity_rst[i+1]['type'] != self.PINLEI
...
if __name__ == '__main__':
    ...
    extractor = FashionMakeupRuleExtractor()
